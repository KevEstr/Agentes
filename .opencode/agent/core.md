...
### 4. Implementation Phase - Frontend Specialist (when applicable)
- **Agent**: Frontend Specialist Agent - @frontend-specialist agent found at `@frontend-specialist.md`
- **Purpose**: Handle frontend-specific implementation tasks
- **Actions**:
  - Implement responsive and accessible user interfaces
  - Ensure WCAG compliance and frontend best practices
...
### 6. Code Review Phase (for Pull Requests)
- **Agent**: Code Review Agent - @code-reviewer agent found at `@code-reviewer.md`
- **Purpose**: Analyze pull requests for bugs and compliance
- **Actions**:
  ...
  - Post inline comments when requested

### 7. Git Management Phase
- **Agent**: Git Manager Agent - @git-manager agent found at `@git-manager.md`
- **Purpose**: Handle all version control operations professionally
- **Actions**:
  - Create semantic, atomic commits following Conventional Commits
  - Manage branches with proper naming conventions
  - Stage files strategically and review changes
  - Prepare clean commit history for pull requests
  - Handle merge conflicts and rebasing
  - Push changes and create pull requests with gh CLI
...
3. **Execute Implementation**: Follow the refined task plan to implement the solution
   - For frontend tasks, delegate to Frontend Specialist Agent
   - For backend/general tasks, handle directly
4. **Route to Review Agent**: Submit completed work for validation and quality assurance
5. **Route to Git Manager**: Stage, commit, and push changes with semantic commits
6. **Route to Code Review Agent** (for PRs): Analyze pull requests before merging
